#!/usr/bin/env python3
"""
run_pipeline.py - Главный скрипт для полного пайплайна квантования ArcFace

Этапы:
1. Загрузка/подготовка FP32 модели (ArcFace)
2. Baseline оценка на валидационном датасете
...
Использование:
    ...
    python run_pipeline.py --dataset ./data/lfw.bin --model ./models/arcface_fp32.onnx
"""
...
# Reuse InsightFace (ArcFace) quantization/evaluation modules
...
def run_full_pipeline(
    ...
    """
    Полный пайплайн квантования и оценки ArcFace

    Args:
        model_path: путь к FP32 ONNX модели (или None для автозагрузки)
        dataset_path: путь к датасету (LFW директория или .bin файл)
    ...
    """
    print("=" * 70)
    print("ArcFace Quantization Pipeline")
    ...
    from export_to_onnx import download_insightface_model
    ...
    if model_path is None:
        print("[INFO] No model provided, downloading InsightFace (ArcFace) buffalo_l...")
        downloaded_path = download_insightface_model(
            model_name="buffalo_l",
            output_dir=str(dirs["models"]),
        )
        fp32_model_path = downloaded_path
    else:
        fp32_model_path = model_path
        print(f"[INFO] Using provided model: {model_path}")

    # Копируем модель в output директорию с arcface-префиксом
    fp32_output = dirs["models"] / "arcface_fp32.onnx"
    ...
    dynamic_model_path = str(dirs["models"] / "arcface_int8_dynamic.onnx")
    ...
    if not skip_static:
        ...
        static_model_path = str(dirs["models"] / "arcface_int8_static_qdq.onnx")
    ...
    if not skip_benchmark:
        ...
        for name, result in benchmark_results.items():
            model_key = name.replace(".onnx", "").replace("arcface_", "")
...
def generate_report(results: Dict, dirs: Dict[str, Path]):
    ...
    with open(report_path, "w") as f:
        f.write("# ArcFace Quantization Evaluation Report\n\n")
        ...
        if "int8_dynamic" in results and "fp32" in results:
            ...
            if dyn_drop < 0.5:
                ...
                f.write(
                    f"Dynamic INT8 quantization shows minimal accuracy degradation ({dyn_drop:.2f}%) "
                )
        ...
        f.write("### Model\n")
        f.write("- Architecture: ArcFace (InsightFace recognition model)\n")
        f.write("- Backbone: depends on model pack (e.g., buffalo_l / buffalo_s)\n")
        f.write("- Embedding dimension: model-dependent (commonly 512)\n\n")

        f.write("### Preprocessing\n")
        f.write("- Input size: 112x112\n")
        f.write("- Color format: RGB\n")
        f.write("- Normalization: (pixel - 127.5) / 127.5 -> range [-1, 1]\n")
        f.write("- Layout: NCHW\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="ArcFace Quantization Pipeline")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        help="Path to FP32 ONNX model (downloads buffalo_l if not provided)",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default=None,
        help="Path to validation dataset (LFW dir or .bin file)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument("--skip-static", action="store_true", help="Skip static quantization")
