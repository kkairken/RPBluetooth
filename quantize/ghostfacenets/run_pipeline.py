#!/usr/bin/env python3
"""
run_pipeline.py - Full quantization pipeline for GhostFaceNets.
"""
...
def run_full_pipeline(
    ...
    print("=" * 70)
    print("GhostFaceNets Quantization Pipeline")
    ...
    export_ghostfacenets_to_onnx = _export_mod.export_ghostfacenets_to_onnx
    ...
    if model_path is None:
        print("[INFO] No model provided, exporting GhostFaceNets to ONNX...")
        fp32_model_path = export_ghostfacenets_to_onnx(
            model_key="w1.3_s1_arcface_v1.2",
    ...
    fp32_output = dirs["models"] / "ghostfacenets_fp32.onnx"
    ...
    dynamic_model_path = str(dirs["models"] / "ghostfacenets_int8_dynamic.onnx")
    quantize_dynamic(
    ...
    if not skip_static:
        ...
        static_model_path = str(dirs["models"] / "ghostfacenets_int8_static_qdq.onnx")
    ...
    if not skip_benchmark:
        ...
        for name, result in benchmark_results.items():
            model_key = name.replace(".onnx", "").replace("ghostfacenets_", "")
...
def generate_report(results: Dict, dirs: Dict[str, Path]):
    ...
    with open(report_path, "w") as f:
        f.write("# GhostFaceNets Quantization Evaluation Report\n\n")
        ...
        f.write("### Model\n")
        f.write("- Architecture: GhostFaceNets (GhostNet-based face model)\n")
        f.write("- Default weights: GhostFaceNet_W1.3_S1_ArcFace (official release)\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="GhostFaceNets Quantization Pipeline")
