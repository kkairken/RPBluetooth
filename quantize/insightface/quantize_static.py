#!/usr/bin/env python3
"""
quantize_static.py - Статическое квантование ONNX модели InsightFace
...
Преимущества:
+ Лучшее ускорение (особенно на специализированных accelerators)
+ Полное INT8 выполнение на поддерживающем hardware
+ Совместимость с TensorRT, OpenVINO

Недостатки:
- Требует калибровочные данные
- Риск деградации качества выше
...
class InsightFaceCalibrationDataReader:
    """
    CalibrationDataReader для InsightFace модели
...
def quantize_static(
    ...
    """Применение статического квантования к ONNX модели"""
    ...
    data_reader = InsightFaceCalibrationDataReader(
...
def load_calibration_from_binary(bin_path: str, n_samples: int = 500) -> List[np.ndarray]:
    """Загрузка калибровочных данных из бинарного файла"""
    from datasets import BinaryDataset
    
    dataset = BinaryDataset(bin_path)
    return dataset.get_calibration_images(n_samples)


def main():
    parser = argparse.ArgumentParser(description="Static ONNX Quantization for InsightFace")
    ...
    parser.add_argument("--calibration-data", type=str, required=True,
                        help="Path to calibration data (LFW directory or .bin file)")
    ...
    if args.calibration_data.endswith(".bin"):
        raw_images = load_calibration_from_binary(args.calibration_data, args.n_calibration)
    else:
