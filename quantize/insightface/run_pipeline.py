#!/usr/bin/env python3
"""
run_pipeline.py - Главный скрипт для полного пайплайна квантования InsightFace

Этапы:
1. Загрузка/подготовка FP32 модели
2. Baseline оценка на валидационном датасете
3. Динамическое квантование + оценка
4. Статическое квантование + оценка
5. Бенчмаркинг производительности
6. Генерация отчёта

Использование:
    python run_pipeline.py --dataset ./data/lfw --output-dir ./results
    python run_pipeline.py --dataset ./data/lfw.bin --model ./models/arcface.onnx
"""
...
def run_full_pipeline(
    ...
    """
    Полный пайплайн квантования и оценки
    ...
    """
    print("="*70)
    print("InsightFace Quantization Pipeline")
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*70)
    
    # Импорты локальных модулей
    from eval_verification import evaluate_model_on_dataset, VerificationMetrics
    from quantize_dynamic import quantize_dynamic, analyze_model_for_quantization
    from quantize_static import quantize_static, prepare_calibration_data
    ...
    if model_path is None:
        print("[INFO] No model provided, downloading InsightFace buffalo_l...")
        from export_to_onnx import download_insightface_model, validate_onnx_model
        
        fp32_model_path = download_insightface_model(
    ...
    fp32_output = dirs['models'] / "insightface_fp32.onnx"
    if str(fp32_model_path) != str(fp32_output):
    ...
    dynamic_model_path = str(dirs['models'] / "insightface_int8_dynamic.onnx")
    ...
    if not skip_static:
        ...
        static_model_path = str(dirs['models'] / "insightface_int8_static_qdq.onnx")
        ...
        if dataset_path:
            ...
            if dataset_path.endswith(".bin"):
                from quantize_static import load_calibration_from_binary
                raw_images = load_calibration_from_binary(dataset_path, n_calibration)
            else:
                from quantize_static import load_calibration_from_lfw
    ...
    if not skip_benchmark:
        ...
        for name, result in benchmark_results.items():
            model_key = name.replace('.onnx', '').replace('insightface_', '')
...
def generate_report(results: Dict, dirs: Dict[str, Path]):
    ...
    with open(report_path, 'w') as f:
        f.write("# InsightFace Quantization Evaluation Report\n\n")
        ...
        if 'int8_dynamic' in results and 'fp32' in results:
            ...
            if dyn_drop < 0.5:
                ...
                f.write("### ⚠️ Consider Alternative Approaches\n\n")
    ...
    # Также сохраняем JSON с полными результатами
...
def main():
    parser = argparse.ArgumentParser(description="InsightFace Quantization Pipeline")
