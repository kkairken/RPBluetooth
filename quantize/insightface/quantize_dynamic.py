#!/usr/bin/env python3
"""
quantize_dynamic.py - Динамическое квантование ONNX модели InsightFace
...
- Хорошо работает для MatMul/Gemm операций (FC layers)

Преимущества:
+ Простое в применении
+ Не требует калибровки
+ Хорошее сжатие модели

Недостатки:
- Меньшее ускорение по сравнению со статическим квантованием
- Overhead на динамическую квантизацию активаций
"""
...
def quantize_dynamic(
    ...
    extra_opts.setdefault('DefaultTensorType', TensorProto.FLOAT)
    
    # Оптимизация модели перед квантованием (рекомендуется)
    if optimize_model:
        print("[INFO] Optimizing model before quantization...")
        
        # Создаём временный файл для оптимизированной модели
        ...
        # Запускаем сессию для оптимизации
...
def analyze_model_for_quantization(model_path: str):
    ...
    # Операции, которые хорошо квантуются динамически
    dynamic_friendly = ['MatMul', 'Gemm', 'Conv']
    
    # Операции, которые лучше работают со статическим квантованием
...
def main():
    parser = argparse.ArgumentParser(description="Dynamic ONNX Quantization for InsightFace")
    ...
    parser.add_argument("--reduce-range", action="store_true",
                        help="Reduce quantization range (7-bit)")
    parser.add_argument("--no-optimize", action="store_true",
                        help="Skip model optimization before quantization")
