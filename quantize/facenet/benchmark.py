#!/usr/bin/env python3
"""
benchmark.py - Бенчмаркинг производительности ONNX моделей FaceNet
...
Использование:
    python benchmark.py --models models/facenet_fp32.onnx models/facenet_int8_dynamic.onnx
"""
...
class ONNXBenchmark:
    """Бенчмаркер для ONNX моделей FaceNet"""
    ...
    def _create_dummy_input(self, batch_size: int = 1) -> np.ndarray:
        """Создание тестового входа (FaceNet: 160x160)"""
        shape = list(self.input_info.shape)
        shape[0] = batch_size
        # FaceNet input: (N, 3, 160, 160)
        shape = [s if isinstance(s, int) else 160 for s in shape]
...
def main():
    parser = argparse.ArgumentParser(description="FaceNet ONNX Model Benchmarking")
